...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Get rid of cells that are known to be safe or have already been selected
        neighbor_cells -= self.safes
        neighbor_cells -= self.moves_made

        # Add new sentence
        self.knowledge.append(Sentence(neighbor_cells, count))

        # Update safes and mines based on the AI's knowledge
        self.update_sentence()

        # Check for new inference
        new_inference = self.new_inference()
        while new_inference:
            for sentence in new_inference:
                # Add to knowledge new sentence after inference
            ...
            for sentence2 in self.knowledge:
                if len(sentence2.cells) == 0:
                    ...
                    if sentence2.cells.issubset(sentence1.cells):
                        inf_cells = sentence1.cells - sentence2.cells
                        inf_count = sentence1.count - sentence2.count

                        new_inference = Sentence(inf_cells, inf_count)
